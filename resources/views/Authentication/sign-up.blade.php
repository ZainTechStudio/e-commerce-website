...
<html lang="en-US" dir="ltr" data-navigation-type="default" data-navbar-horizontal-shape="default">
  ...
  <head>
    ...
    <!--    Favicons-->
    <!-- ===============================================-->
    <link rel="apple-touch-icon" sizes="180x180" href="../../../assets/img/favicons/favicon-16x16.png">
        ...
        <div class="row flex-center min-vh-100 py-5">
          <div class="col-sm-10 col-md-8 col-lg-5 col-xl-5 col-xxl-3"><a class="d-flex flex-center text-decoration-none mb-4" href="../../../index.html">
            ...
            <div class="position-relative mt-5">
              ...
              <div class="divider-content-center">sign-up</div>
            </div>
            <form method="POST" action="{{ url('/Auth/sign-up') }}">
            @csrf
              <div class="mb-3 text-start"><label class="form-label" for="name">Name</label><input value="@if(isset($old['name'])){{ $old['name'] }}@else{{ old('name') }}@endif" class="form-control" name="name" id="name" type="text" placeholder="Name" />
              ...
              <div class="mb-3 text-start"><label class="form-label" for="phoneNumber">Phone Number</label><input value="@if(isset($old['phoneNumber'])){{ $old['phoneNumber'] }}@else{{ old('phoneNumber') }}@endif" class="form-control" name="phoneNumber" id="phoneNumber" type="text" placeholder="+xx xxxxxxxxxx" />
                <span class="text-danger">
                  @if (isset($phoneNumErr))
                  {{ $phoneNumErr }}
                  @else
                  @error('phoneNumber')
                  {{ $message }}
              ...
              <div class="mb-3 mt-3 text-start"><label class="form-label" for="recoveryEmail">Recovery email address</label><input value="@if(isset($old['recoveryEmail'])){{ $old['recoveryEmail'] }}@else{{ old('recoveryEmail') }}@endif" class="form-control" name="recoveryEmail" id="recoveryEmail" type="email" placeholder="name@example.com" />
                <span class="text-danger">
                  @if (isset($recoveryEmailErr))
                  {{ $recoveryEmailErr }}
                  @else
                  @error('recoveryEmail')
                  {{ $message }}
              ...
              <div class="mb-3 text-start"><label class="form-label" for="email">Email address</label><div class="input-group"><input value="@if(isset($old['email'])) {{ $old['email'] }} @else {{ old('email') }} @endif" class="form-control" name="email" id="email" type="text" placeholder="name@gmail.com" />
              ...
              <div class="row g-3 mb-3">
                <div class="col-sm-6"><label class="form-label" for="password">Password</label>
                  <div class="position-relative" data-password="data-password"><input value="@if(isset($old['password_confirmation'])){{$old['password_confirmation']}}@else{{old('password_confirmation')}}@endif" class="form-control form-icon-input pe-6" name="password" id="password" type="password" placeholder="Password" data-password-input="data-password-input" /><button type="button" class="btn px-3 py-0 h-100 position-absolute top-0 end-0 fs-7 text-body-tertiary" data-password-toggle="data-password-toggle"><span class="uil uil-eye show"></span><span class="uil uil-eye-slash hide"></span></button></div>
                  <span class="text-danger">
                    @if (isset($passErr))
                    {{ $passErr }}
                    @else
                ...
                <div class="col-sm-6"><label class="form-label" for="confirmPassword">Confirm Password</label>
                  <div class="position-relative" data-password="data-password"><input value="@if(isset($old['password_confirmation'])){{$old['password_confirmation']}}@else{{old('password_confirmation')}}@endif" class="form-control form-icon-input pe-6" name="password_confirmation" id="confirmPassword" type="password" placeholder="Confirm Password" data-password-input="data-password-input" /><button type="button" class="btn px-3 py-0 h-100 position-absolute top-0 end-0 fs-7 text-body-tertiary" data-password-toggle="data-password-toggle"><span class="uil uil-eye show"></span><span class="uil uil-eye-slash hide"></span></button></div>
                  <span class="text-danger">
                    @if (isset($passConfErr))
                    {{ $passConfErr }}
                    @else
              ...
              <div class="form-check mb-3"><input class="form-check-input" name="terms" id="termsService" type="checkbox" /><label class="form-label fs-9 text-transform-none" for="termsService">I accept the <a href="#">terms </a>and <a href="#!">privacy policy</a></label><br>
                <span class="text-danger">
                  @if (isset($termsErr))
                  {{ $termsErr }}
                  @else
              ...
              <div class="text-center"><a class="fs-9 fw-bold" href="{{ route('sign-in_page') }}">Sign in to an existing account</a></div>
            </form>
          </div>
        </div>
      </div>
